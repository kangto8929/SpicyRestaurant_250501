...
public class ShapesArray
{
    ...
    private ConstantsSO constantsData; // Add a reference to ConstantsSO

    public ShapesArray(ConstantsSO constantsData)
    {
        this.constantsData = constantsData;
        shapes = new GameObject[constantsData.Rows, constantsData.Columns]; // Initialize with ConstantsSO data
    }
    ...
    public MatchesInfo GetVerticalMatch(GameObject go)
    {
        MatchesInfo matchesInfo = new MatchesInfo();
        var verticalMatches = GetMatchesVertically(go);
        verticalMatches = GetEntireColumn(go);
    ...
    public MatchesInfo GetHorizontalMatch(GameObject go)
    {
        //아이템 클릭 시 세로형 폭발
        MatchesInfo matchesInfo = new MatchesInfo();
        var horizontalMatches = GetMatchesHorizontally(go);
    ...
    public MatchesInfo GetSurroundingMatches(GameObject go)
    {
        ...
        for (int row = shape.Row - 1; row <= shape.Row + 1; row++)
        {
            ...
            {
                if (row >= 0 && row < constantsData.Rows && column >= 0 && column < constantsData.Columns) // Use ConstantsSO
                {
                    var item = shapes[row, column];
                    if (item != null)
                    {
    ...
    {
        if (matches.Count() >= constantsData.MinimumMatches) // Use ConstantsSO
        {
    ...
    private IEnumerable<GameObject> GetEntireRow(GameObject go)
    {
        ...
        for (int column = 0; column < constantsData.Columns; column++) // Use ConstantsSO
        {
    ...
    private IEnumerable<GameObject> GetEntireColumn(GameObject go)
    {
        ...
        for (int row = 0; row < constantsData.Rows; row++) // Use ConstantsSO
        {
            matches.Add(shapes[row, column]);
        }
        return matches;
    ...
    private IEnumerable<GameObject> GetMatchesHorizontally(GameObject go)
    {
        ...
        //check right
        if (shape.Column != constantsData.Columns - 1) // Use ConstantsSO
            for (int column = shape.Column + 1; column < constantsData.Columns; column++) // Use ConstantsSO
            {
                if (shapes[shape.Row, column].GetComponent<Shape>().IsSameType(shape))
                {
                    matches.Add(shapes[shape.Row, column]);
    ...
    private IEnumerable<GameObject> GetMatchesVertically(GameObject go)
    {
        ...
        //check top
        if (shape.Row != constantsData.Rows - 1) // Use ConstantsSO
            for (int row = shape.Row + 1; row < constantsData.Rows; row++) // Use ConstantsSO
            {
                if (shapes[row, shape.Column] != null && 
                    shapes[row, shape.Column].GetComponent<Shape>().IsSameType(shape))
                {
    ...
    public AlteredCandyInfo Collapse(IEnumerable<int> columns)
    {
        ...
        foreach (var column in columns)
        {
            ...
            for (int row = 0; row < constantsData.Rows - 1; row++) // Use ConstantsSO
            {
                //if you find a null item
                if (shapes[row, column] == null)    
                {
                    //start searching for the first non-null
                    for (int row2 = row + 1; row2 < constantsData.Rows; row2++) // Use ConstantsSO
                    {
